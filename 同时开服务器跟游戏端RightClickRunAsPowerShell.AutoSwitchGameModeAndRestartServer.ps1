...
function RestartGameServer {  
    ...
    Start-Process -FilePath "${gameRootPath}\Rumbleverse\Binaries\Win64\RumbleverseClient-Win64-Shipping.exe" -ArgumentList "-log", "-nullrhi", "-notexturestreaming", "-threads 200", "-high", "-noaudio" -WindowStyle hidden # -PassThru
        ...
        # 获取最新启动的进程
<#    $latestProcess = Get-Process -Name $processName | Sort-Object StartTime -Descending | Select-Object -First 1
    $global:latestPID = $latestProcess.Id
    Write-Host "Latest Process PID: $latestPID is still running."
#>
...
#>
    ...
    # 筛选出路径包含 $gameRootPath 的进程
$latestProcess = Get-Process -Name $processName -ErrorAction SilentlyContinue | Where-Object { $_.Path -and $_.Path.StartsWith($gameRootPath) } | Sort-Object StartTime -Descending | Select-Object -First 1

# 如果找到了匹配的进程，赋值给 $latestPID
if ($latestProcess) {
    $latestPID = $latestProcess.Id
    Write-Host "找到了服务器端的进程，ID为: $latestPID"
} else {
    Write-Host "未找到服务器端的进程"
}
...
#>
     #设置服务端进程优先级为高
    Get-WmiObject Win32_Process -Filter "ProcessId=$latestPID" | ForEach-Object { $_.SetPriority(128) | Out-Null }
    ...
    $command = ".\Injector2.exe -p $latestPID --inject $gameModeFile"
...
if ($process) {
    ...
    Write-Output "[$timestamp] Process $processName is listening on port $listeningPort." | Tee-Object -FilePath $guardian_rumbleverse_log -Append
    Write-Host "[$timestamp] Process $processName is listening on port $listeningPort."
        if ($joinCommand -notmatch "62169") {
                $warningMessage = "`r`n[$timestamp] 游戏服务端已经启动，但检测端口发现不是默认的62169，如果你也没有修改过端口配置，【那可能是配置文件出问题了，去这个目录(..\Rumbleverse\Binaries\Win64\Config.ini)删除配置文件,关了当前窗口再开脚本试一次，还不行就运行 [服务器设置(改模式和等待时间).请先运行这个再开服.bat] 这个后再重新运行开服脚本.】 `r`n"
